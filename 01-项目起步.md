...
> 以 `@` 开头的包通常是**作用域包**（Scoped Packages）。这意味着这些包属于一个特定的组织、团队或者用户.
>
> 在 TypeScript 项目中使用 ESLint 进行代码检查时，可能会遇到一些特定于 TypeScript 的配置问题，例如 TypeScript 的特定规则、解析器、插件等。`@rushstack/eslint-patch` 提供了一种简单的方法来解决这些问题。
>
> 这个工具会对 ESLint 进行一些必要的补丁，以确保它可以正确地处理 TypeScript 代码。这样，你就可以在 TypeScript 项目中使用 ESLint，而无需担心配置问题。
