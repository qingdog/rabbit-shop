...
> InstanceType 是 TypeScript 中的内置类型，它接受一个类（或类型）作为参数，返回该类的实例类型。
...
> declare module 通过ts声明模块，你可以在 TypeScript 中对引入的 vue 模块进行类型检查和提供相应的类型提示，以确保在代码编写过程中可以获得更好的类型支持。
>
> ```bash
> src\components\vk-data-goods-sku-popup\vk-data-goods-sku-popup.d.ts:141:declare module '@vue/runtime-core' {
> ```
>
> export interface 导出一个全局组件接口，该接口将用于定义在 Vue.js 项目中全局可用的组件。
